...
#---------------------------------------------------------------------#
##LOAD MVMR DATA
MVMR_BMI_CSI <- read.xlsx("results/MVMR_results/MVMR_BMI_CSI.xlsx", rowNames = T)

names(MVMR_BMI_CSI)[names(MVMR_BMI_CSI)=="Estimate"] <- "b"
names(MVMR_BMI_CSI)[names(MVMR_BMI_CSI)=="Std..Error"] <- "se"
names(MVMR_BMI_CSI)[names(MVMR_BMI_CSI)=="Pr(>|t|)"] <- "pval"
names(MVMR_BMI_CSI)[names(MVMR_BMI_CSI)=="exposure.1"] <- "exposure"
MVMR_BMI_CSI <- MVMR_BMI_CSI[grepl("BMI", rownames(MVMR_BMI_CSI)), ]

MVMR_BMI_CSI$method <- "Multivariable MR"

MVMR_BMI_CSI <- subset(MVMR_BMI_CSI, select = -c(t.value, `F-statistic`, exposure.2))
...
results_BMI <- rbind(univar_BMI, MVMR_BMI_CSI)
...
save_forest_func("results/plots/mvmr_plots/mvmr_bmi_csi.png", p_mvmr)
